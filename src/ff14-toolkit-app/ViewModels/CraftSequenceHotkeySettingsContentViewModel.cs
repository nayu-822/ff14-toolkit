...
public sealed class CraftSequenceHotkeySettingsContentViewModel : ShellContentViewModel
{
    ...
    private void RefreshAvailableSequences()
    {
        ...
        foreach (CraftSequenceHotkeySlotViewModel slot in HotkeySlots)
        {
            ...
            CraftSequenceOptionViewModel? selectedOption = AvailableSequences
                .FirstOrDefault(option => option.SequenceId == slot.SelectedSequenceId);
            slot.UpdateSelectedSequenceOption(selectedOption ?? AvailableSequences.FirstOrDefault());
        }
    }
